...
namespace LibRT {
	...
	class LIBRTSHARED_EXPORT OAuth10aAuthenticator : public virtual OAuthAuthenticator
	{
		public:
			...
			/// @fn QByteArray getOAuthToken() const;
			/// @brief Getting the OAuth Token
			/// @return The OAuth Token
			QByteArray getOAuthToken() const;
			...
			/// @fn QByteArray getVerifier() const;
			/// @brief Getter on the verifier
			/// @return The verifier
			QByteArray getVerifier() const;
			...
			/// @fn QByteArray getAuthorizationHeader(LibRT::HTTPRequestType type,
			///										  QString baseURL,
			///										  ArgsMap getDatas,
			///										  ArgsMap postDatas,
			///										  bool oauthTokenNeeded = true,
			///										  bool callbackUrlNeeded = false,
			///										  bool oauthVerifierNeeded = false) const;
		...
		protected:
			...
			/// @fn QByteArray signDatas(LibRT::RequestType type,
			///							 QString baseURL,
			///							 ArgsMap getDatas,
			///							 ArgsMap postDatas,
			///							 QString nonce,
			...
			QByteArray signDatas(HTTPRequestType type,
								 QString baseURL,
								 ArgsMap getDatas,
								 ArgsMap postDatas,
								 QString nonce,
			...
			///
			/// It is used for the signature (string with the OAuth parameters)
			/// and for the Authorization Header
			/// @param nonce Nonce of the request
			/// @param timestamp Timestamp of the request
			...
			/// @param oauthVerifierNeeded Boolean indicating whether
			/// the oauth_verifier is required.
			/// @param putDoubleQuotes Boolean indicating if double quotes have
			/// to surround the percent encoded value.
