...
///
/// Its <code>executeTests();</code> method execute all the unit tests coded in
/// this class, using the <code>QTest::qExec();</code> function on itself.
class TestCase : public TestEntity
{
	...
	private slots:
		...
		///
		/// Increments nbTests and the number of failures (if the test failed,
		/// of course) and then executes @c tearDownAfterClass();
