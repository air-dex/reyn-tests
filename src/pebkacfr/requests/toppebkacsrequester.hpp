/// @file toppebkacsrequester.hpp
/// @brief Header of TopPebkacsRequester
...
#ifndef TOPPEBKACSREQUESTER_HPP
#define TOPPEBKACSREQUESTER_HPP
...
namespace LibRT {
	namespace Pebkac {
		/// @class TopPebkacsRequester
		/// @brief Requester for Top Pebkacs
		/// @see http://dev.pebkac.fr/doc/top/
		class TopPebkacsRequester : public PebkacRequester
		{
			public:
				/// @fn TopPebkacsRequester(const PebkacAuthenticator & authPebkac,
				...
				/// mocks for tests.
				TopPebkacsRequester(const PebkacAuthenticator & authPebkac,
									int offset = OFFSET_DEFAULT_VALUE,
									int limit = LIMIT_DEFAULT_VALUE,
									QString period = PEBKAC_DEFAULT_PERIOD,
									QNetworkAccessManager & netManager = LIBRT_NETWORK_MANAGER);
